...
function Assert-ManualAndReadmeClean {
    ...
    Assert-TextMatches (Join-Path $repoRoot 'src\ClipmanApplicationContext.cs') 'new HistoryForm\(store, settings, SaveSettings, RegisterHotkeys' 'Entry Properties can refresh Quick Copy hotkey registrations'
    Assert-TextMatches (Join-Path $repoRoot 'src\HistoryForm.cs') 'QuickCopyAssignmentChanged' 'Quick Copy assignment changes are detected'
    Assert-TextMatches (Join-Path $repoRoot 'src\HistoryForm.cs') 'refreshHotkeys\(\);' 'Quick Copy assignment changes refresh registered hotkeys'
    Assert-TextDoesNotMatch $manual 'Fixed Windows Quick Copy assignments made from Entry Properties' 'Manual does not describe new Quick Copy behavior as a prior Windows fix'
    Assert-TextDoesNotMatch $readme 'Fixed Windows Quick Copy assignments made from Entry Properties' 'README does not describe new Quick Copy behavior as a prior Windows fix'
    ...
    Assert-TextMatches (Join-Path $repoRoot 'src\HistoryForm.cs') 'SubItems\.Add\(entry\.Pinned \? "Pinned" : string\.Empty\)' 'Pinned text state remains in pinned column'
