...
function Get-TextFiles([string]$root) {
    $extensions = @(
        ...
        '.json', '.md', '.ps1', '.sh', '.sln', '.swift', '.txt', '.xml', '.yml', '.yaml'
    )
    ...
    Get-ChildItem -LiteralPath $root -Recurse -File -Force |
        Where-Object {
            $_.FullName -notmatch '\\(\.git|release|bin|obj|portable|dist|\.build)(\\|$)' -and
...
function Test-TextFile([string]$path, [string]$displayPath) {
    ...
    $claudeMarker = 'CLAU' + 'DE' + '\.md'
    $handoverMarker = 'HAND' + 'OVER' + '\.md'
    $planMarker = 'PL' + 'AN-[^\s]+\.md'
    ...
    $checks = [ordered]@{
        'private Windows local path' = ('(?<![A-Za-z0-9])(?:[DEF]:' + [regex]::Escape($bs) + ')')
        'local user profile path' = ('C:' + [regex]::Escape($bs) + 'Users' + [regex]::Escape($bs) + $privateUser)
        ...
        'public source contains private handover file names' = ($claudeMarker + '|' + $handoverMarker + '|' + $planMarker)
    }
...
function Test-Zip([string]$zipPath) {
    ...
    $extractRoot = Join-Path ([IO.Path]::GetTempPath()) ('Clipman-release-privacy-' + [guid]::NewGuid().ToString('N'))
