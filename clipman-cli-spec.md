...
| <code>rules</code> command group and channel-aware sync per <code>sync-rules-spec.md</code> | Complete (section 7.13) |
...
| Go-generated sync-rules fixture set with Go, Windows, Mac, and Android consumers | Complete (section 14.1) |
...
- <code>sync-rules-spec.md</code> for sync channels: derivations, the rules
  document schema, routing, relocation, and the multi-channel algorithm.
...
identity, merge, model, operation, sync rules (<code>internal/rules</code>),
sync engine (including the multi-channel engine in
<code>internal/syncengine/channels.go</code>) — must be applied to both trees
...
### 7.13 <code>rules</code> — sync channels

~~~text
clipman-cli rules show [--json]
clipman-cli rules enable [--json]
clipman-cli rules disable [--json]
clipman-cli rules channel add <name> [--group G]... [--source-device D]... [--kind richtextimages] [--json]
clipman-cli rules channel remove <name> [--json]
clipman-cli rules device set <device> --channels <k1,k2|*> [--json]
~~~

**Shipped.** Manages the machine-scoped sync rules of
<code>sync-rules-spec.md</code>, which is normative for every derivation,
schema, and algorithm below; this section documents only the CLI surface.

<code>rules show</code> prints the document in effect — channels with a
plain-language route summary, then registered devices with their
subscriptions — or "Sync rules are not enabled." With <code>--json</code> it
emits the raw document object (or <code>null</code>). A document written by a
newer format version is shown with a note that only what this version
understands is applied; every mutating subcommand refuses such a document,
because a client that cannot fully evaluate the rules must not rewrite them.

<code>rules enable</code> and <code>rules disable</code> flip the document's
global <code>Enabled</code> switch, creating the rules bucket on first enable
with <code>If-None-Match: *</code>. Disabling never moves or deletes entries;
every client simply returns to single-bucket behavior.

<code>rules channel add</code> creates a channel whose route ANDs the given
conditions (repeat <code>--group</code> and <code>--source-device</code> for
lists). Names are validated against the channel-key grammar and reserved
names of <code>sync-rules-spec.md</code> section 3 before anything is
uploaded. <code>rules channel remove</code> implements the spec's
channel-deletion algorithm: it first re-routes the channel's resident entries
under a transitional document (they fall through to the next matching rule or
to core, via the ordinary gain-before-lose two-phase upload), verifies the
re-route actually committed and left the channel empty, and only then removes
the channel from the rules document. A concurrent newer rules edit aborts the
removal rather than risking entries stranded in an unlisted channel.

<code>rules device set</code> replaces one device's subscription list, adding
the device to the registry if it is not yet listed. <code>--channels</code>
takes channel names or the single value <code>*</code> meaning every channel;
core is always implicit. Mixing <code>*</code> with named channels is
rejected.

Channel-aware behavior of the other commands: <code>list</code>,
<code>get</code>, <code>put</code>, <code>rm</code>, <code>sync</code>,
<code>pick</code>, and <code>menu</code> operate on the merged view of core
plus the channels the configured <code>machine</code> name subscribes to,
through the engine's <code>ReadView</code>/<code>MutateView</code>. With no
rules document, or a disabled one, behavior is byte-for-byte the pre-rules
single-bucket flow. A capture that routes to an unsubscribed channel is
written through to that channel's bucket and reported on one line; if the
write-through fails, the entry is parked in a pending store beside the
configuration file and retried by later commands. The last-seen rules
document is cached beside the configuration file as well (spec section 4,
Caching); neither file carries a secret. A future-version document is cached
for display only and is never re-uploaded by the 404 fallback.
...
ClipmanCli/
  ...
  cmd/clipman-cli/rules.go       + rules_test.go — rules commands, caches
  internal/model/                model.go, file_history.go
  internal/clipdb/               CLIPDB1/CLIPDB2 codec, raw rules-blob container
  internal/identity/             bucket ID derivation (database, channel, rules)
  ...
  internal/rules/                sync rules document, validation, routing
  internal/syncengine/           read + optimistic mutate; channels.go multi-channel view
...
- The sync-rules set (<code>go/</code>): a rules-document blob, per-channel
  blobs, channel and rules identity vectors, and the merged view a subscribed
  device must assemble, generated by the Go reference and asserted by the Go
  engine, the Windows regression harness, and Mac and Android unit tests. See
  the corpus README's "The sync-rules set" section.
