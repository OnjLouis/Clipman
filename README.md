...
Clipman Server has its own update path. Windows and macOS server wrappers include a Check for Updates menu item and command-line switches such as `--check-updates` and `--install-update`. Installed Linux servers provide `clipmanserver check-update` and `clipmanserver update`, plus an optional daily systemd timer. Linux updates require the GitHub SHA-256 asset digest, are staged and health-checked, and automatically restore the previous program if the updated service cannot start. Every platform updates only server program files; settings, databases, logs, backups, certificates, and tokens remain in their per-user data folders.
