...
- **One history on the devices you choose.** Windows and Mac can share a history through a cloud-synced folder or network share. Clipman Server extends encrypted synchronization to every supported platform without requiring a third-party clipboard account. Optional sync rules can split that shared history into named channels routed by group, source device, or embedded images, so each device downloads only the channels it subscribes to.
