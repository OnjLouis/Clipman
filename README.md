...
- **Quick Paste removes the detour.** Assign a global hotkey to a frequently used entry and paste it into the active application without opening history. Templates can resolve dates, device information, and other fields at the moment they are used. On desktop, optional ClipMerge can also append a text or file selection when you deliberately copy it twice in quick succession.
