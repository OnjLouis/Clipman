...
- **One history on the devices you choose.** Windows and Mac can share a history through a cloud-synced folder or network share. Clipman Server extends encrypted synchronization to every supported platform without requiring a third-party clipboard account. Optional sync rules can split that shared history into named channels routed by group, source device, or embedded images, so each device downloads only the channels it subscribes to. Channels organize and reduce transfers; clients sharing one token and history password can access every channel, so channels are not an access-control boundary.
...
- **Quick Paste removes the detour.** Assign a global hotkey to a frequently used entry and paste it into the active application without opening history. Templates can resolve dates, device information, and other fields at the moment they are used. On desktop, optional ClipMerge can also append text or file selections when you deliberately copy or cut the next selection twice in quick succession; cuts merge only while every source path still exists.
- **Quick Clip captures a thought directly.** Create and synchronize a new entry from Clipman without first copying it in another app. Desktop clients use `Ctrl+N` or `Command+N` and can assign an optional global Quick Clip hotkey; mobile clients expose the same editor beside Paste.
...
4. Use Quick Clip when you want to type a new saved entry directly, or open Preferences or Settings when you want to enable optional views, encryption, synchronization, sounds, or other behavior.
...
- **Clipman Server:** Self-host a server on Linux, macOS, Windows, Raspberry Pi, a NAS, or a VPS. One token can serve multiple separate histories: clients with the same history password sync together, while different passwords select different database buckets. This is not per-user server access control. [See how sharing one server works](https://onjlouis.github.io/Clipman/server/manual.html#sharing-a-server). Internet-facing servers must use HTTPS.
